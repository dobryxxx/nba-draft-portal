...
function StatusBadge({ children, color = '#7c5ccf', bg = 'rgba(238,233,251,.75)' }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-white/35 px-3 py-2 font-mono text-[9px] font-black uppercase tracking-[.16em] backdrop-blur-md" style={{ color, background: bg }}>
...
function PlayerAvatar({ prospect, size = 'md' }) {
  const img = getPlayerImage(prospect)
  ...
  return (
    <div 
      className={dimension + ' flex shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/35 font-sans text-lg font-extrabold backdrop-blur-md'} 
      style={{ 
    ...
    >
      {img ? <img src={img} alt={prospect.name} className="h-full w-full object-contain object-bottom p-1" draggable="false" /> : initials}
    </div>
  )
}
...
function CommandHero({ onNavigate, prospectCount }) {
  return (
    <motion.header
      ...
      initial="hidden"
      animate="show"
      className="relative overflow-hidden rounded-[42px] border border-white/40 bg-white/35 px-7 py-8 shadow-[0_24px_70px_rgba(0,0,0,.08)] backdrop-blur-xl md:px-10 md:py-10"
    >
      <motion.span animate={{ x: [0, 12, 0], y: [0, -8, 0] }} transition={{ duration: 8, repeat: Infinity, ease: 'easeInOut' }} className="pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full bg-[#eee9fb] blur-3xl" />
      <motion.span animate={{ x: [0, -10, 0], y: [0, 10, 0] }} transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut' }} className="pointer-events-none absolute -bottom-24 left-12 h-72 w-72 rounded-full bg-[#edf7fd] blur-3xl" />
      ...
      <div className="relative flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-6 font-sans text-5xl font-extrabold leading-none tracking-tight text-slate-800 md:text-7xl">
            Draft Command Center
          </h1>
          <p className="mt-5 max-w-2xl text-lg font-semibold leading-8 text-slate-600/80">
            Simule o Draft. Analise cenarios. Tome decisoes como um front office.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <motion.button
              ...
              whileTap={{ scale: .98 }}
              className="rounded-full px-7 py-4 font-mono text-[11px] font-black uppercase tracking-[.2em] text-white shadow-[0_18px_42px_rgba(124,92,207,.22)]"
            ...
            <motion.button
              ...
              whileHover={{ y: -3, scale: 1.02 }}
              whileTap={{ scale: .98 }}
              className="rounded-full border border-white/45 bg-white/30 px-7 py-4 font-mono text-[11px] font-black uppercase tracking-[.2em] text-slate-700 shadow-[0_12px_30px_rgba(0,0,0,.05)] backdrop-blur-md"
            >
              Explorar Big Board
            </motion.button>
          </div>
        ...
        <div className="grid min-w-[240px] gap-3 rounded-[30px] border border-white/35 bg-white/24 p-4 backdrop-blur-md">
...
function HeroMetric({ label, value, color }) {
  return (
    <div className="flex items-center justify-between gap-5 rounded-2xl bg-white/28 px-4 py-3">
...
function QuickActions({ onNavigate }) {
  const actions = [
    { id: 'mockdraft', title: 'Mock Draft', icon: Target, color: '#7c5ccf', description: 'Entre na War Room, rode a loteria e tome decisoes pick a pick.' },
...
function QuickActionCard({ id, title, description, icon: Icon, color, onNavigate }) {
  return (
    <motion.button
      ...
      whileTap={{ scale: .985 }}
      className="group relative min-h-[210px] overflow-hidden rounded-[34px] border border-white/35 bg-white/30 p-6 text-left shadow-[0_16px_48px_rgba(0,0,0,.06)] backdrop-blur-xl transition-all duration-300"
    >
      ...
      <div className="relative flex h-full flex-col justify-between">
        ...
        <div className="mt-7">
          <h2 className="font-sans text-2xl font-extrabold tracking-tight text-slate-800">{title}</h2>
...
function DraftIntel({ prospects = [] }) {
  ...
  return (
    <motion.section variants={sectionMotion} initial="hidden" animate="show" className="rounded-[36px] border border-white/35 bg-white/24 p-6 shadow-[0_18px_56px_rgba(0,0,0,.06)] backdrop-blur-xl">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="font-mono text-[9px] font-black uppercase tracking-[.28em] text-lo">INTEL</div>
          <h2 className="mt-1 font-sans text-3xl font-extrabold tracking-tight text-slate-800">Draft Intel</h2>
        </div>
...
function IntelCard({ icon: Icon, title, body, color }) {
  return (
    <motion.div variants={sectionMotion} whileHover={{ y: -3, scale: 1.01 }} className="rounded-[26px] border border-white/25 bg-white/22 p-4 backdrop-blur-md transition-all duration-300 hover:bg-white/32 hover:shadow-[0_14px_34px_rgba(0,0,0,.06)]">
...
function TopBoardPreview({ prospects = [], onNavigate }) {
  ...
  return (
    <motion.section variants={sectionMotion} initial="hidden" animate="show" className="rounded-[36px] border border-white/35 bg-white/30 p-6 shadow-[0_18px_56px_rgba(0,0,0,.06)] backdrop-blur-xl">
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          <div className="font-mono text-[9px] font-black uppercase tracking-[.28em] text-lo">BOARD</div>
          <h2 className="mt-1 font-sans text-3xl font-extrabold tracking-tight text-slate-800">Top Board Preview</h2>
        </div>
...
function BoardRow({ prospect, index, onNavigate }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: .99 }}
      className="grid items-center gap-4 rounded-[26px] border border-white/25 bg-white/20 p-4 text-left backdrop-blur-md transition-all duration-300 hover:bg-white/34 hover:shadow-[0_14px_34px_rgba(0,0,0,.055)] md:grid-cols-[58px_56px_minmax(0,1fr)_140px_130px]"
...
export default function Home({ onNavigate, prospectCount, prospects = [] }) {
  return (
    <div className="relative min-h-full overflow-hidden px-6 py-8" style={{ background: '#edeae4' }}>
      <span className="pointer-events-none absolute right-0 top-0 h-96 w-96 rounded-full bg-[#eee9fb]/70 blur-3xl" />
      <span className="pointer-events-none absolute bottom-16 left-0 h-96 w-96 rounded-full bg-[#edf7fd]/70 blur-3xl" />
