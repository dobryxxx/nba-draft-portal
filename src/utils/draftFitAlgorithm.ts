...
export type PickCompatibilityStatus = 'ideal' | 'reachable' | 'reach' | 'unlikely_available' | 'no_pick';

export interface PlayerExpectedRange {
  minPick: number;
  maxPick: number;
  expectedPick: number;
  tier: PlayerRangeTier;
}

export interface PickCompatibility {
  bestPick?: NormalizedDraftPick;
  compatibility: number;
  status: PickCompatibilityStatus;
  explanation: string;
}

export interface DraftFitResult {
  teamId: TeamId;
  teamName: string;
  score: number;
  label: 'Elite Draft Fit' | 'Strong Draft Fit' | 'Good Draft Fit' | 'Weak Draft Fit' | 'Blocked Fit';
  realism: 'High' | 'Medium' | 'Low' | 'Blocked';
  pickContext: string;
  reasons: string[];
  flags: string[];
  components: {
  ...
  debug?: {
    playerRange: PlayerExpectedRange;
    pickCompatibility: PickCompatibility;
  };
}

export type DraftFitOptions = {
  limit?: number;
...
type DraftProspect = {
  ...
  bigBoardRank?: number;
  mockRank?: number;
  draftRange?: string | number[] | { min?: number; max?: number; minPick?: number; maxPick?: number; expected?: number; expectedPick?: number };
  projectedRange?: string | number[] | { min?: number; max?: number; minPick?: number; maxPick?: number; expected?: number; expectedPick?: number };
  tier?: string;
  age?: number;
  position?: string;
  height?: string;
...
type AttributeKey = keyof TeamNeeds;

const ATTRIBUTE_KEYS: AttributeKey[] = ['shooting', 'creation', 'defense', 'rebounding', 'athleticism', 'size', 'floor', 'ceiling'];
const DEFAULT_WEIGHTS = { draftRangeFit: 0.40, availabilityFit: 0.25, teamNeedFit: 0.20, teamStrategyFit: 0.15 };
const REALISM_ORDER: Record<DraftFitResult['realism'], number> = { High: 0, Medium: 1, Low: 2, Blocked: 3 };

const clamp = (value: number, min = 0, max = 100): number => Math.min(max, Math.max(min, value));
const clamp01 = (value: number): number => Math.min(1, Math.max(0, value));
const round = (value: number): number => Math.round(clamp(value));
const pct = (value: number): number => round(value * 100);
const toScore = (value: unknown, max = 10): number => typeof value === 'number' && Number.isFinite(value) ? clamp((value / max) * 100) : 0;

function stat(player: DraftProspect, key: string): number | undefined {
  const value = player.stats?.[key];
  return typeof value === 'number' && Number.isFinite(value) ? value : undefined;
}

function attr(player: DraftProspect, ...keys: string[]): number | undefined {
  ...
  for (const key of keys) {
    ...
    if (found) return found[1] as number;
  }
  return undefined;
}

function role(position = ''): 'guard' | 'wing' | 'big' {
...
function playerRank(player: DraftProspect): number {
...
export function getPlayerDraftAttributes(player: DraftProspect): TeamNeeds {
  ...
  const shooting = average([
    toScore(attr(player, 'Shooting', 'shooting')),
    clamp(((stat(player, 'threep') ?? stat(player, 'threePct') ?? 32) - 25) * 5),
    clamp(((stat(player, 'ftp') ?? stat(player, 'ft') ?? stat(player, 'ftPct') ?? 70) - 55) * 2.4),
    clamp(((stat(player, 'ts') ?? 54) - 48) * 5.6),
    wordScore(scoutingText, ['shoot', 'spacing', 'arremesso', '3'], ['preocupante', 'inconsistente']),
  ]);
  const creation = average([
    toScore(attr(player, 'Playmaking', 'creation', 'playmaking')),
    clamp(((s.apg ?? 2.5) / 7) * 100),
    clamp(((s.astTo ?? 1.2) / 3) * 100),
    clamp(((s.usg ?? 20) - 12) * 4.2),
    wordScore(scoutingText, ['creator', 'cria', 'handle', 'playmaking', 'passing'], ['turnover', 'decisao']),
  ]);
  const defense = average([
    toScore(attr(player, 'Defense', 'defense')),
    clamp(((s.stlPct ?? 1.7) / 4.5) * 100),
    clamp(((s.blkPct ?? 1.8) / 8) * 100),
    wordScore(scoutingText, ['defense', 'defesa', 'switch', 'rim protection', 'steal'], ['defensivo preocupante', 'lento']),
  ]);
  const rebounding = average([
    toScore(attr(player, 'Rebounding', 'rebounding')),
    clamp(((s.rpg ?? 4.5) / (playerRole === 'big' ? 11 : 7)) * 100),
    wordScore(scoutingText, ['rebound', 'rebote', 'motor'], ['rebounding concern']),
  ]);
  const athleticism = average([
    toScore(attr(player, 'Athleticism', 'athleticism')),
    clamp(50 + Math.max(0, wingspan - height) * 5 + (age <= 19 ? 8 : 0) + (playerRole !== 'guard' ? 4 : 0)),
    wordScore(scoutingText, ['athletic', 'explosivo', 'transition', 'vertical', 'speed'], ['below average athlete']),
  ]);
  const size = average([
    toScore(attr(player, 'Size', 'size')),
    clamp(((height - 72) / 13) * 70 + Math.max(0, wingspan - height) * 5 + (playerRole !== 'guard' ? 10 : 0)),
    wordScore([player.height, player.wingspan, scoutingText].join(' '), ['size', 'length', 'longo', 'wingspan'], ['undersized']),
  ]);
  const floor = average([
    toScore(attr(player, 'Floor', 'floor')),
    clamp((s.ts ?? 54) * 1.25 - 25),
    clamp((s.per ?? 18) * 3.5),
    shooting * 0.20 + defense * 0.25 + creation * 0.15 + 35,
  ]);
  const ceiling = average([
    toScore(attr(player, 'Ceiling', 'ceiling')),
    clamp((100 - rank) + 18),
    clamp(((s.ppg ?? 10) / 24) * 100),
    clamp(((s.usg ?? 18) - 12) * 4.5),
...
function explicitRange(player: DraftProspect): PlayerExpectedRange | undefined {
  const raw = player.draftRange ?? player.projectedRange;
  if (!raw) return undefined;
  let minPick: number | undefined;
  let maxPick: number | undefined;
  let expectedPick: number | undefined;
  if (Array.isArray(raw)) {
    minPick = Number(raw[0]);
    maxPick = Number(raw[1] ?? raw[0]);
  } else if (typeof raw === 'object') {
    minPick = Number(raw.minPick ?? raw.min);
    maxPick = Number(raw.maxPick ?? raw.max);
    expectedPick = Number(raw.expectedPick ?? raw.expected);
...
export function getPlayerExpectedRange(player: DraftProspect): PlayerExpectedRange {
...
function pickDistanceToRange(pick: number, range: PlayerExpectedRange): number {
  if (pick >= range.minPick && pick <= range.maxPick) return 0;
  return pick < range.minPick ? range.minPick - pick : pick - range.maxPick;
}

function scorePickForRange(pick: NormalizedDraftPick, range: PlayerExpectedRange): PickCompatibility {
  ...
  if (pickNumber >= range.minPick && pickNumber <= range.maxPick) {
    const compatibility = clamp01(1 - (expectedDistance / (span + 4)) * 0.18);
    return { bestPick: pick, compatibility, status: 'ideal', explanation: `Pick #${pickNumber} dentro do range esperado (#${range.minPick}-#${range.maxPick}).` };
  }
...
export function getPickCompatibility(playerRange: PlayerExpectedRange, teamPicks: NormalizedDraftPick[]): PickCompatibility {
  const firstRoundPicks = teamPicks.filter(pick => pick.round === 1 && pick.pick <= 30);
  if (!firstRoundPicks.length) {
    return { compatibility: 0, status: 'no_pick', explanation: 'Sem escolha de primeira rodada compativel.' };
  }
  const scored = firstRoundPicks.map(pick => scorePickForRange(pick, playerRange));
  const statusRank: Record<PickCompatibilityStatus, number> = { ideal: 0, reachable: 1, reach: 2, unlikely_available: 3, no_pick: 4 };
  return scored.sort((a, b) => statusRank[a.status] - statusRank[b.status] || b.compatibility - a.compatibility)[0];
}

export function getDraftRangeFit(player: DraftProspect, teamPicks: NormalizedDraftPick[]): number {
  return pct(getPickCompatibility(getPlayerExpectedRange(player), teamPicks).compatibility);
}

export function getAvailabilityFit(player: DraftProspect, teamPicks: NormalizedDraftPick[]): number {
  const compatibility = getPickCompatibility(getPlayerExpectedRange(player), teamPicks);
  const map: Record<PickCompatibilityStatus, number> = {
    ideal: 95,
    reachable: 78,
    reach: 66,
    unlikely_available: 28,
    no_pick: 0,
  };
...
function weightedAverage(values: Record<AttributeKey, number>, weights: Record<AttributeKey, number>): number {
...
function calculateTeamNeedFit(player: DraftProspect, teamProfile: TeamProfile): number {
  const attrs = getPlayerDraftAttributes(player);
  const weightedNeeds = ATTRIBUTE_KEYS.reduce((acc, key) => {
    acc[key] = teamProfile.needs[key] * (teamProfile.playerAttributeWeights[key] || 0);
    return acc;
  }, {} as Record<AttributeKey, number>);
  return round(weightedAverage(attrs, weightedNeeds));
}

function priorityScore(priority: Priority, attrs: TeamNeeds): number {
  const map: Record<Priority, number> = {
    spacing: attrs.shooting,
    ...
    size: attrs.size,
    floor: attrs.floor,
    ceiling: attrs.ceiling,
    two_way: (attrs.defense + attrs.shooting + attrs.size) / 3,
    frontcourt_depth: (attrs.size + attrs.rebounding + attrs.defense) / 3,
  };
  return map[priority] ?? 50;
}

function modeScore(mode: DraftMode, attrs: TeamNeeds): number {
  const map: Record<DraftMode, number> = {
    talent_accumulation: attrs.ceiling * 0.55 + attrs.creation * 0.25 + attrs.athleticism * 0.20,
    upside_swing: attrs.ceiling * 0.55 + attrs.athleticism * 0.25 + attrs.size * 0.20,
    core_builder: attrs.ceiling * 0.35 + attrs.creation * 0.25 + attrs.shooting * 0.15 + attrs.defense * 0.15 + attrs.floor * 0.10,
    playoff_support: attrs.floor * 0.32 + attrs.defense * 0.24 + attrs.shooting * 0.22 + attrs.size * 0.12 + attrs.creation * 0.10,
    win_now_support: attrs.floor * 0.38 + attrs.shooting * 0.24 + attrs.defense * 0.22 + attrs.creation * 0.10 + attrs.rebounding * 0.06,
    contender_depth: attrs.floor * 0.40 + attrs.defense * 0.24 + attrs.shooting * 0.22 + attrs.rebounding * 0.08 + attrs.ceiling * 0.06,
  };
  return map[mode] ?? 50;
}

function riskScore(teamProfile: TeamProfile, attrs: TeamNeeds): number {
...
function timelineScore(teamProfile: TeamProfile, attrs: TeamNeeds): number {
...
function calculateTeamStrategyFit(player: DraftProspect, teamProfile: TeamProfile): number {
  const attrs = getPlayerDraftAttributes(player);
...
export function getRealismLabel(player: DraftProspect, teamPicks: NormalizedDraftPick[]): DraftFitResult['realism'] {
  const compatibility = getPickCompatibility(getPlayerExpectedRange(player), teamPicks);
...
export function getDraftFitLabel(score: number, isBlocked: boolean): DraftFitResult['label'] {
...
function needReason(teamProfile: TeamProfile, attrs: TeamNeeds): { reason: string; flags: string[] } {
  ...
  const labels: Record<AttributeKey, string> = { shooting: 'arremesso/spacing', creation: 'criacao com bola', defense: 'defesa', rebounding: 'rebote', athleticism: 'atletismo', size: 'tamanho', floor: 'piso', ceiling: 'teto' };
  return { reason: `O perfil de ${labels[top.key]} responde uma prioridade relevante do time.`, flags };
}

function strategyFlags(teamProfile: TeamProfile, attrs: TeamNeeds): string[] {
  const flags: string[] = [];
...
export function generateDraftFitReasons(
  player: DraftProspect,
  teamProfile: TeamProfile,
  fitComponents: DraftFitResult['components'],
  pickCompatibility: PickCompatibility,
  ...
  if (pickCompatibility.status === 'ideal' && pick) {
    reasons.push(`O time possui a escolha #${pick.pick}, dentro do range esperado do jogador (#${range.minPick}-#${range.maxPick}).`);
    flags.push('Pick range match', 'Likely available');
  } else if (pickCompatibility.status === 'reachable' && pick) {
    reasons.push(`A escolha #${pick.pick} fica perto do range esperado (#${range.minPick}-#${range.maxPick}), mantendo uma janela realista.`);
    flags.push('Likely available');
  } else if (pickCompatibility.status === 'reach' && pick) {
    reasons.push(`A escolha #${pick.pick} seria acima do range esperado do jogador (#${range.minPick}-#${range.maxPick}).`);
    flags.push('Reach risk');
  } else if (pickCompatibility.status === 'unlikely_available' && pick) {
    reasons.push(`A escolha #${pick.pick} fica tarde demais para um prospecto projetado entre #${range.minPick}-#${range.maxPick}.`);
    flags.push('Unlikely available');
  } else {
    reasons.push('Sem pick compativel na primeira rodada, o encaixe fica bloqueado pelo draft capital.');
    flags.push('Blocked by draft capital');
  }
  ...
  if (editorial) reasons.push(`Estrategia editorial: ${editorial}`);
  else reasons.push('A estrategia editorial do time conversa com este tipo de aposta no draft.');

flags.push(...strategyFlags(teamProfile, attrs));
  return { reasons: [...new Set(reasons)].slice(0, 3), flags: [...new Set(flags)].slice(0, 6) };
}

function scoreCap(status: PickCompatibilityStatus): number {
...
function pickContext(range: PlayerExpectedRange, compatibility: PickCompatibility): string {
...
export function calculateDraftFit(player: DraftProspect, teamProfile: TeamProfile, currentOrder?: DraftOrderInput): DraftFitResult {
  ...
  const cappedScore = Math.min(weightedScore, scoreCap(pickCompatibility.status));
  ...
  return {
    ...
    score,
    label: getDraftFitLabel(score, blocked),
    realism,
    pickContext: pickContext(playerRange, pickCompatibility),
    reasons,
    flags,
    components,
    debug: { playerRange, pickCompatibility },
  };
}

export function getBestDraftFits(player: DraftProspect, options: DraftFitOptions = {}): DraftFitResult[] {
...
// Example for future UI usage:
// const fits = getBestDraftFits(prospect, { limit: 5, currentOrder: resolvedPicks });
