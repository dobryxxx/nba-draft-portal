...
import { formatStat, getAllNormalizedStatValues, normalizeProspectStats, normalizeStatValue } from './prospectStats'
...
export const num = (v, d = 1) => formatStat(v, d)
...
export const quality = (k, v, p) => { const value=normalizeStatValue(k, v); if (typeof value !== 'number') return COLORS.miss; const r = role(p.position); const [a,s] = thresholds[r]?.[k] || thresholds[k] || [0,1]; return value >= s ?COLORS.strong : value >= a ?COLORS.avg : COLORS.weak }
export const metricMax = (k, fallback) => Math.max(fallback, ...getAllNormalizedStatValues(ALL_PROSPECTS, k))
export const metricNote = (k, v, p, fallbackMax) => { const value=normalizeStatValue(k, v); if (typeof value !== 'number') return 'sem dado confiavel na base'; const r=role(p.position), roleLabel=r === 'big' ?'big' : r === 'forward' ?'forward' : r === 'wing' ?'ala' : 'guard', [avg,strong]=thresholds[r]?.[k] || thresholds[k] || [fallbackMax*.45,fallbackMax*.7], max=metricMax(k,fallbackMax); if (value >= strong) return `nivel alto para ${roleLabel} · topo DB ${num(max)}`; if (value >= avg) return `faixa media/boa · topo DB ${num(max)}`; return `abaixo da referencia · topo DB ${num(max)}` }
export const profileHeadline = p => PROFILE_TITLES[p.name] || (() => { const s=p.stats||{}; if (['CORNERSTONE', 'ELITE'].includes(p.tier)) return 'Talento de topo com caminho para estrela'; if ((s.ts||0) >= 60 && (s.usg||0) >= 24) return 'Criador eficiente de alto volume'; if ((s.threep||0) >= 38) return 'Especialista de spacing NBA'; if ((s.blkPct||0) >= 3 || (s.rpg||0) >= 9) return 'Interior de impacto fisico'; if ((s.astTo||0) >= 2 || (s.apg||0) >= 5) return 'Condutor secundario de rotação'; return 'Perfil NBA em desenvolvimento' })()
...
export const manualTranslationSummary = p => { const direct=p?.scouting?.translationSummary; const resolved=resolvedProspectIntelligence(p)?.scouting?.translationSummary; const value=typeof direct==='string'&&direct.trim()?direct:resolved; return typeof value==='string'&&value.trim()?value.trim():null }
...
export function estimateOutcomeScores(p) { const s=p.stats||{}, highTier=['CORNERSTONE','ELITE'].includes(p.tier), floor=Math.round(clamp(38+(s.ppg||0)*.7+(s.ts||50)*.35,35,92)), ceiling=Math.round(clamp(floor+14+(highTier?8:4),45,98)); return { floor: Math.min(floor, ceiling-1), ceiling } }
export function resolveOutcomeScores(p) { const estimated=estimateOutcomeScores(p), mf=manualFloorScore(p), mc=manualCeilingScore(p), highTier=['CORNERSTONE','ELITE'].includes(p.tier); let floor=mf ?? estimated.floor, ceiling=mc ?? estimated.ceiling; if(mf !== null && mc === null) ceiling=Math.max(estimated.ceiling, floor + (highTier?20:14)); if(mc !== null && mf === null) floor=Math.min(estimated.floor, ceiling - 10); floor=Math.round(clamp(floor,35,98)); ceiling=Math.round(clamp(ceiling,36,99)); if(floor >= ceiling) floor=Math.max(35, ceiling-1); return { floor, ceiling } }
...
export function getPlayerComp(p) { const s=p.stats||{}, r=role(p.position), highTier=['CORNERSTONE','ELITE'].includes(p.tier), floor=r==='guard'?'Rotation Guard':r==='big'?'Rotation Big':'Rotation Wing'; if((s.threep||0)>=38) return ['Ceiling: premium spacer', 'Floor: '+floor]; if((s.ppg||0)>=20) return ['Ceiling: primary scorer', 'Floor: microwave scorer']; if((s.blkPct||0)>=3.5) return ['Ceiling: defensive anchor', 'Floor: rim protection specialist']; return ['Ceiling: '+(highTier?'franchise upside':'starter upside'), 'Floor: '+floor] }
export function getOverviewCopy(p) { const s=p.stats||{}, sub=(s.ts||0)>=60?'Produz volume com eficiência acima da curva.':(s.threep||0)>=37?'O arremesso cria um caminho claro de tradução.':Math.max(s.stlPct||0,s.blkPct||0)>=3?'Defesa e ferramentas fisicas puxam o valor inicial.':'Perfil que combina contexto, desenvolvimento e papel NBA.', notes=Array.isArray(p.scouting?.notes)?p.scouting.notes.join(' '):p.scouting?.notes; return { headline: profileHeadline(p), subheadline: getPlayerArchetype(p)+' / '+sub, body: manualTranslationSummary(p) || notes || 'Leitura baseada em producao, tier e indicadores disponiveis. O encaixe ideal depende de papel ofensivo claro e paciencia no desenvolvimento.' } }
...
export function getStoryTags(p) { const s=p.stats||{}, tags=[]; if(['CORNERSTONE','ELITE'].includes(p.tier)) tags.push('Franchise Swing'); if((s.ppg||0)>=20) tags.push('Elite Scorer'); if((s.threep||0)>=37) tags.push('Shooting Swing'); if(Math.max(s.stlPct||0,s.blkPct||0)>=3) tags.push('Defensive Bet'); if((s.astTo||0)>=1.8 || (s.apg||0)>=4) tags.push('Playmaking Upside'); if((p.rank||99)>24) tags.push('Development Project'); if(!tags.length) tags.push('Safe Floor'); return tags.slice(0,5) }
